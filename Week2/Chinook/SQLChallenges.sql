...
SELECT 
    CONCAT(FirstName, ' ', LastName) AS Full_name, CustomerId, Country
FROM 
    Customer
WHERE 
    country != 'USA';
...
SELECT COUNT(InvoiceId) AS NumberOfInvoices, SUM(Total) AS SalesTotal, 
YEAR(InvoiceDate) AS InvoiceYear 
FROM Invoice WHERE YEAR(InvoiceDate) = 2009 GROUP BY YEAR(InvoiceDate);

    -- (challenge: find the invoice count sales total for every year using one query)
    SELECT COUNT(InvoiceID) AS InvoiceCount, Sum(Total) AS SalesTotal, YEAR(InvoiceDate) AS InvoiceYear   FROM Invoice GROUP BY YEAR(InvoiceDate);
...
SELECT BillingCountry, COUNT(*) AS NumberOfInvoices From Invoice GROUP BY BillingCountry ORDER BY NumberOfInvoices DESC, BillingCountry;
...
SELECT t1.Name AS ArtistName, t2.Title AS AlbumTitle FROM Artist AS t1 JOIN Album AS t2 ON t1.ArtistId = t2.ArtistId;
...
SELECT TOP 1
    t.TrackId,
    t.Name AS Track,
    SUM(il.Quantity) AS QtyPurchased
FROM
    InvoiceLine il
JOIN
...
SELECT TOP 3
    t4.Name,
    SUM(t1.Quantity * t1.UnitPrice) AS TotalSales
FROM
    InvoiceLine AS t1
JOIN
    Track AS t2 ON t1.TrackId = t2.TrackId
JOIN
    Album AS t3 ON t2.AlbumId = t3.AlbumId
JOIN
    Artist AS t4 ON t3.ArtistId = t4.ArtistId
GROUP BY
    t4.Name
ORDER BY
    TotalSales DESC;
...
AND
    substring(t1.LastName, 1, 1) = substring(t2.LastName, 1, 1)
ORDER BY
...
WHERE NOT EXISTS (
  ...
  WHERE al.ArtistId = ar.ArtistId AND g.Name = 'Latin'
)
ORDER BY ar.Name;
...
FROM
    Track AS t1
JOIN
    MediaType AS t2 ON t1.MediaTypeId = t2.MediaTypeId
WHERE
    t2.Name LIKE '%video%'
ORDER BY
    t1.Milliseconds DESC;
...
JOIN
    Track AS t4
ON
    t1.TrackId = t4.TrackId
JOIN
    MediaType AS t5
ON
    t4.MediaTypeId = t5.MediaTypeId
WHERE
    t3.Country = 'Germany'
AND
    t5.Name = 'MPEG audio file';
...
FROM
    Customer AS t1
JOIN
    Employee AS t2 ON t1.SupportRepId = t2.EmployeeId
WHERE
    DATEDIFF(year, t2.BirthDate, t2.HireDate) < 35;
-- DML exercises
